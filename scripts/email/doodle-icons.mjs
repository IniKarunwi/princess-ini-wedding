/**
 * The doodle vocabulary — six icons, drawn once, used everywhere.
 *
 * Extracted from generate-backdrop.mjs so the flourishes printed INSIDE the
 * card and the tile behind it are the same drawings rather than two sets that
 * drift apart. generate-backdrop.mjs scatters them at 3–8.5% opacity around
 * the page; generate-doodles.mjs arranges them at full strength into the small
 * images the letter actually shows.
 *
 * Each is stroke-only in a 100×100 box, origin at the centre. The paths are
...
export const ORDER = ['rings', 'bouquet', 'champagne', 'heart', 'envelope', 'sprig'];
